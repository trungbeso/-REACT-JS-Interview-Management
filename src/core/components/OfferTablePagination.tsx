...
interface OfferTablePaginationProps {
  dataSource: any[];
        ...
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-white">
          ...
          <tbody>
            {dataSource?.map((item: any, index: number) => (
              ...
              >
                {columns?.map((column: any, index: number) => {
                  if (column.title === 'Department' && item[column.field]) {
                    return (
                      <td key={index} className="px-6 py-4">
                        {item[column.field].name || 'N/A'}
                      </td>
                    );
                  }

                  if (column.title === 'Candidate Name' && item[column.field]) {
                    return (
                      <td key={index} className="px-6 py-4">
                  ...
                  if (column.title === 'Approver' && item[column.field]) {
                    return (
                      <td key={index} className="px-6 py-4">
                        {item[column.field].fullName || 'N/A'}
                      </td>
...
export default OfferTablePagination;
